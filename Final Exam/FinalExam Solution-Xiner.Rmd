...
> If compare results from 1B.a with 1B.c, that is 1C.a, for employees without other sources of insurance, emploer-provided health insurance lowers the probability of seeking new employment opportunities, average difference of expected probabilities is -0.1686. The 95% CI is (-0.2214, -0.1209), it doesn't include 0, thus we can reject the null and conclude that job lock has a significant effect for employees without other health insurance, at 5% significance level.

> If compare results from 1B.b with 1B.d, for employees with other health insurance, employer-provided
# health insurance also lowers the probability of seeking new employment opportunities, avergae difference of expected probabilities is -0.1098. The 95% CI is (-0.1579, -0.0655 ), it doesn't include 0, thus we can reject the null and conclude that job lock has a significant effect even for employees with other health insurance, at 5% significance level.
...
It makes sense because employees with other sources of insurance tends to less worry about losing employer-provided health insurance simply because they have backup plans, thus job is less a constrain (but still is a constrain) or factor when considering new employment opportunities.
...
**Write a function in R that evaluates the log-likelihood.**
...
**Using your log-likelihood function and optim(), run a tobit regression where durable is the dependent
...
## 2.D 
**Generate a histogram of 10,000 expected values of Y_i with quant at it's 20th percentile value
...
**Generate a histogram of 10,000 expected values with quant at it's 80th percentile value and age at its mean.**
...
**What is the average first difference and its standard error?**
...
> Why 80% percentile of quant has larger observed expected outcome Y_i than 20% percentile of quant? The effect estimate is -0.0457. I think, efect estimate is a measure of latent variable which is left-censored, the expected latent variable for 80% percentile is actually smaller than 20% percentile, but after censoring all negative values to 0, we actully shift more data in 80% percentile group to the right, which makes the obvserved average expected outcome larger than the other group.  
